...
def plot( event, DVModel ):
    ...
    for j in range(len(DVModel)):
        ...
        im = ax.pcolor(d,v,ans - d - v,vmin=-4,vmax=0)
        ax.plot( d[0] , DVModel[j].line(d[0]) , 'r', linewidth = 2, 
            label = r"${:.2f}\cdot D^{}{:.3f}{}$".format( 10**DVModel[j].iterc, "{", DVModel[j].slope, "}" ) )
        if( DVModel[j].slope > 0.55 ): 
    ...
    fig.savefig( 'charts/mdd_test_lnorm_{}'.format(event))
    plt.show()
...
def subpop_plot( event, group_scores ):
    ...
    fig.savefig( 'charts/mdd_test_lnorm_subpop_{}'.format(event) )
    plt.show()